...
class Security
{
    ...
    protected function shouldUseLibreSSL()
    {
        if ($this->_useLibreSSL === null) {
            ...
            $this->_useLibreSSL = defined('OPENSSL_VERSION_TEXT')
                && preg_match('{^LibreSSL (\d\d?)\.(\d\d?)\.(\d\d?)$}', OPENSSL_VERSION_TEXT, $matches) && ((10000 * $matches[1]) + (100 * $matches[2]) + $matches[3]) >= 20105;
        }
    ...
    public function hashData($data, $key, $rawHash = false)
    {
        ...
        if (!$hash) {
            throw new RuntimeException('Failed to generate HMAC with hash algorithm: ' . $this->macHash);
        }
     ...
     * @throws RuntimeException if wrong length is specified
     ...
     * @throws RuntimeException on bad password/hash parameters or if crypt() with Blowfish hash is not available.
     * @see generatePasswordHash()
     */
     ...
     * @throws \RuntimeException|\Exception if the cost parameter is out of the range of 4 to 31.
     */
    protected function generateSalt(int $cost = 13): string
    {
        if ($cost < 4 || $cost > 31) {
            throw new RuntimeException('Cost must be between 4 and 31.');
        }
    ...
    public function compareString($expected, $actual)
    {
        if (!is_string($expected)) {
            throw new RuntimeException('Expected expected value to be a string, ' . gettype($expected) . ' given.');
        }

        if (!is_string($actual)) {
            throw new RuntimeException('Expected actual value to be a string, ' . gettype($actual) . ' given.');
        }
